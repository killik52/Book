...
class CriarNovoArtigoActivity : AppCompatActivity() {
    ...
    private fun verificarArtigoBloqueadoPorSerial(numeroSerialArtigo: String, callback: (Boolean, ClienteBloqueado?) -> Unit) {
        ...
        val db = ClienteDbHelper(this).readableDatabase
        ...
        val tableExists = try {
            ...
            Log.e("CriarNovoArtigo", "Erro ao verificar se tabela fatura_items existe: ${e.message}")
            false
        }

        if (!tableExists) {
            Log.w("CriarNovoArtigo", "Tabela fatura_items não existe, pulando verificação de artigo bloqueado")
            callback(false, null)
            return
        }
