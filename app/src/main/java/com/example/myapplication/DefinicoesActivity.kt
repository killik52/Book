...
class DefinicoesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            val limparBancoRow: LinearLayout = findViewById(R.id.limparBancoRow)
            limparBancoRow.setOnClickListener {
                Log.d("DefinicoesActivity", "Limpar Banco row clicado")
                val intent = Intent(this, LimparBancoActivity::class.java)
                startActivity(intent)
                overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left)
            }

            val limparClientesRecentesRow: LinearLayout = findViewById(R.id.limparClientesRecentesRow)
            limparClientesRecentesRow.setOnClickListener {
                Log.d("DefinicoesActivity", "Limpar Clientes Recentes row clicado")
                val intent = Intent(this, LimparBancoActivity::class.java).apply {
                    putExtra("limpar_apenas_clientes", true)
                }
                startActivity(intent)
                overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left)
            }
    ...
    private fun mostrarDialogoConfirmacaoLimparClientes() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Sim, Limpar") { _, _ ->
                limparClientesRecentes()
            }
            .setNegativeButton("Cancelar", null)
            .show()
    }

    private fun limparClientesRecentes() {
        try {
            // 1. Limpar clientes do banco SQLite
            val db = dbHelper?.writableDatabase
            if (db != null) {
                db.beginTransaction()
                try {
                    val deletedRows = db.delete("clientes", null, null)
                    Log.d("DefinicoesActivity", "Clientes removidos do SQLite: $deletedRows")
                    
                    // Resetar contador de ID
                    db.execSQL("DELETE FROM sqlite_sequence WHERE name='clientes'")
                    
                    db.setTransactionSuccessful()
                    Log.d("DefinicoesActivity", "Clientes limpos do SQLite com sucesso")
                } catch (e: Exception) {
                    Log.e("DefinicoesActivity", "Erro ao limpar clientes do SQLite: ${e.message}", e)
                } finally {
                    db.endTransaction()
                }
            }

            // 2. Limpar clientes do banco Room (em background)
            lifecycleScope.launch {
                try {
                    val database = AppDatabase.getDatabase(this@DefinicoesActivity)
                    database.clienteDao().deleteAllClientes()
                    Log.d("DefinicoesActivity", "Clientes limpos do Room com sucesso")
                    
                    runOnUiThread {
                        showToast("Clientes recentes limpos com sucesso!")
                    }
                } catch (e: Exception) {
                    Log.e("DefinicoesActivity", "Erro ao limpar clientes do Room: ${e.message}", e)
                    runOnUiThread {
                        showToast("Erro ao limpar clientes do Room: ${e.message}")
                    }
                }
            }

        } catch (e: Exception) {
            Log.e("DefinicoesActivity", "Erro geral ao limpar clientes: ${e.message}", e)
            showToast("Erro ao limpar clientes: ${e.message}")
        }
    }

    override fun onBackPressed() {
        Log.d("DefinicoesActivity", "onBackPressed disparado.")
