    ...
    private fun aplicarMascaras() {
        ...
        cnpjTextWatcher = object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) {
                ...
                val formatted = when {
                    str.length > 12 -> "${str.substring(0, 2)}.${str.substring(2, 5)}.${str.substring(5, 8)}/${str.substring(8, 12)}-${str.substring(12)}"
                    str.length > 8 -> "${str.substring(0, 2)}.${str.substring(2, 5)}.${str.substring(5, 8)}/${str.substring(8)}"
                    str.length > 5 -> "${str.substring(0, 2)}.${str.substring(2, 5)}.${str.substring(5)}"
                ...
                if (clienteAtual.id > 0) {
                    ...
                    if (rowsUpdated != null && rowsUpdated > 0) {
                        ...
                        if (indexInList != -1) {
                            ...
                            val mudou = if (listaNomesClientesBloqueados[indexInList] != nome) {
